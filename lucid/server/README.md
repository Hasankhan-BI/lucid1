...
- **LLM:** Anthropic's Claude API, called **only from the server** — the
...
npm install
cp .env.example .env
# edit .env: set JWT_SECRET to a random string, and ANTHROPIC_API_KEY to your key
npm start
```
...
Get an Anthropic API key at https://console.anthropic.com — the app runs
...
public/            Static frontend (index.html + app.js). Talks to the
                    backend over fetch(); never calls Anthropic directly.
...
- **No billing/usage metering yet.** Nothing stops a user from running up
  your Anthropic bill — add this before letting strangers use it for free.
...
**3. Set service variables** (Variables tab): `JWT_SECRET`, `ANTHROPIC_API_KEY`,
...
cd server
cp .env.example .env
nano .env   # set JWT_SECRET (long random string) and ANTHROPIC_API_KEY
```
...
- The Anthropic API key lives only in `.env` on the server — it is never
